...
def find_password():
    ...
    # open file if it exists and display username
    # display error if file does not exist, or site not in file
    try:
        with open(FILE) as f:
            data = json.load(f)
    except FileNotFoundError:
        ...
        if site in data:
            username = data[site].get("email")
            password = data[site].get("password")
            title = site
            message = f"Username: {username}\nPassword: {password}"
        else:
            title = "ERROR"
            message = f"No Details for {site}"
    finally:
...
site_input = tk.Entry(width=18)
site_input.grid(row=1, column=1)
site_input.focus()

search_button = tk.Button(
    text="Search", padx=2, pady=2, width=15, command=find_password
)
